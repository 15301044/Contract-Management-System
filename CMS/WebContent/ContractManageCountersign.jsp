...
<body>
  ...
  <div class="container-manage">
	  ...
	  <script type="text/javascript">
	  	document.getElementById("hqht").setAttribute("class","list-group-item bg active");
	  </script>
      ...
      <%}%>  
          <div class="panel-heading">
            <h3 class="panel-title">待会签合同列表</h3>
          </div>        
          <div class="panel-body"><!--panel body start-->
            <form class="form-horizontal" role="form" action="ContractManageCountersign" method="get">  
            	        
              <div class="form-group">
                <label for="合同名称" class="col-sm-2 control-label">待会签合同名称：</label>
            ...
            <table class="table table-hover table-condensed table-white" style="color:#ffffff;">
                ...
                <% for(int i=0;i<contracts.size();i++){ %>
                	 ...
                	 <td><%=((Contract)contracts.get(i)).GetId()%></td>
                		...
                		<td><a href="ContractManageCountersign?type=detail&id=<%=contracts.get(i).GetId() %>">会签</a></td>
                	</tr>
                <%}%>
                </tbody>
            </table>
            ...
            <div class="form-group">
            	...
            	<a id="pageone" onmouseover="ChangeHref('pageone')" class="col-sm-1 col-sm-offset-1" href="<%if(pageNo!=1){out.print("ContractManageCountersign?pageNo=1");}else{out.print("#");}%>"><span class="glyphicon glyphicon-fast-backward myclickstyle"></span></a>
	      	    <a id="pagebefore" onmouseover="ChangeHref('pagebefore')" class="col-sm-1" href="<% if(pageNo>=2){out.print("ContractManageCountersign?pageNo="+(pageNo-1));}else{out.print("#");} %>"><span class="glyphicon glyphicon-arrow-left myclickstyle"></span></a>
	      	    <span class="col-sm-3"><%=pageNo %>/<%=pageNum %>(共 <%=totalRecordNum%> 条)</span>
	      	    <a id="pagenext" onmouseover="ChangeHref('pagenext')" class="col-sm-1" href="<% if(pageNo+1<=pageNum){out.print("ContractManageCountersign?pageNo="+(pageNo+1));}else{out.print("#");} %>"><span class="glyphicon glyphicon-arrow-right myclickstyle"></span></a>
	      	    <a id="pagefinal" onmouseover="ChangeHref('pagefinal')" class="col-sm-1" href="<%if(pageNo!=pageNum){out.print("ContractManageCountersign?pageNo="+pageNum);}else{out.print("#");}%>"><span class="glyphicon glyphicon-fast-forward myclickstyle"></span></a>
